...
for i in range(len(recordOfTheDay)):	
	...
	# dont include the z
	time = incorrectTime[11:len(incorrectTime)-2]	# should be 11:12 
	...
	if i == 0:
		...
		if sap != 0:
			...
			for i in range(len(arrayOfAllPhoneEventsForTheDay)):
				if arrayOfAllPhoneEventsForTheDay[i][6] == sap and arrayOfAllPhoneEventsForTheDay[i][0] == 'f':
...
file = open(date + "PhoneEventsAddedLocation.txt","w+")
